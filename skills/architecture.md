...
Shared architecture concepts for MVI and MVVM. Load first for architecture questions, then see [mvi.md](mvi.md) or [mvvm.md](mvvm.md) for pattern-specific details, and [design-patterns.md](design-patterns.md) for comprehensive software design patterns in Modern Kotlin & Compose.
...
## State Collection and Slicing (Router Pattern)

**Mandatory Screen Architecture:**
- `*Route` (Stateful Entry Point): Obtains ViewModel via DI (`koinViewModel()`), collects `StateFlow<ScreenState>` using `collectAsStateWithLifecycle()`, observes side-effects, and passes state down.
- `*Screen` (Stateless Content): Receives `ScreenState` and event callbacks (`onIntent`). 100% Stateless and decoupled from ViewModel/Koin.
- `*Screen Preview` (Mandatory): Every `*Screen` MUST provide `@Preview` with mock/fake `ScreenState` wrapped in `MyAppTheme`.
- Leaves receive **only what they need** and provide their own `@Preview` functions.
- Do **not** make leaf composables observe the ViewModel directly.
